...
export const load: PageServerLoad = async () => {
	...
	try {
		...
		// 1. Scan hub airports for actually-delayed flights (DL departure status)
		...
		// 2. If we need more flights to fill 5 slots, pull a few on-time ones from fallback routes
		...
		// Build final list: delayed flights first, pad with non-delayed up to 5
		const picks = [...delayed, ...nonDelayed].slice(0, 5);

		// 3. Pre-fetch full status for all delayed flights (and any landed/departed ones)
